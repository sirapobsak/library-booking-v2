...
- **จับเฉพาะเสียงคน** (`VOICE_ONLY`): `speech_filter` (viper, fixed-point Q12: รวม 2 ค่า -> 8 kHz 16 บิต, เก็บเศษจาก >>12 ไว้บวกรอบหน้า (error feedback) กันเพี้ยน/สั่นค้างตอนเงียบ, ตัด DC, HPF 250 Hz + LPF 3400 Hz) + `VoiceMeter` (พลังงานเหนือ 250 Hz อยู่ในช่วงเสียงพูด ≥ 50% และ p90−p10 ของ dB ช่วงพูดใน 1 วิ ≥ 6 dB — เดิมหารด้วยพลังงานทั้งหมด + stdev ≥ 4 dB ซึ่งพังในห้องจริงที่มีแอร์/พัดลม ~67 dB + เสียงหึ่งต่ำ; `FLOOR_MAX_DB` 72; `esp32/tools/mic_test.py` ใช้ตัวกรอง/เกณฑ์เดียวกัน ต้องแก้คู่กัน) — **แก้สเกลตัวเลขใน viper ต้องเช็ค overflow int 32 บิต** (ชุดทดสอบมีเคสเสียงเกือบเต็มสเกล)
